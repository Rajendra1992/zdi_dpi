...
st.markdown(
    ...
    /* Custom CSS for larger dataframe - Enhanced for Classifications page */
    .classification-dataframe {
        width: 100% !important;
        min-height: 700px !important;
        margin-top: -20px !important; /* Remove blank space above */
    }
    
    /* Make the data editor container full width and height for Classifications */
    ...
    /* Specific styling for the classification data editor table */
    ...
    /* Remove extra spacing in classifications section */
    .classification-editor-container {
        margin-top: -30px !important;
        padding-top: 10px !important;
    }
    
    /* Ensure full width for classification report section */
    .classification-editor-container .block-container {
...
# Snowflake Masking Validation App
if app_mode == "Classifications":
    ...
    if app_mode_classification == "Home":
        ...
        st.markdown('<p>The system now includes auto-save functionality. Any changes you make to the classification report are automatically saved to the backend tables in real-time.</p>', unsafe_allow_html=True)
        st.markdown('<p>After reviewing the entire classification report and making necessary edits (which are auto-saved), you can finally submit the report by selecting a BU name and clicking Submit Classifications.</p>', unsafe_allow_html=True)
        st.markdown('<p>Note: Changes are automatically saved as you edit, but the report must be officially submitted once to complete the process. </p>', unsafe_allow_html=True)
    
    elif app_mode_classification == "Classification edit and Submission":
        # Session state initialization
        for key in ["report_fetched", "edited_df", "submitted", "confirm_submission", "last_save_time", "save_status"]:
            if key not in st.session_state:
                if key == "edited_df":
                    st.session_state[key] = None
                elif key in ["last_save_time", "save_status"]:
        ...
        def auto_save_classification_report(df, database, schema):
            """Auto-save function that saves silently in background"""
            try:
                session = get_active_session()
                values = []
                ...
                # Update session state for save indicator
                current_time = time.strftime("%H:%M:%S")
                st.session_state.last_save_time = current_time
                st.session_state.save_status = "✅ Auto-saved"
                return True
            except Exception as e:
                st.session_state.save_status = f"❌ Save failed"
                return False
        ...
        # Auto-save status indicator (only show if there's a status)
        if st.session_state.save_status:
            st.markdown(f'<div class="auto-save-indicator">{st.session_state.save_status} at {st.session_state.last_save_time}</div>', unsafe_allow_html=True)
 
        # Database and Schema selection in columns for better space utilization
        ...
        with col3:
            if database and schema:
                if st.button("Get Classification Report", use_container_width=True):
        ...
        # Editable DataFrame with enhanced layout
        if st.session_state.report_fetched and st.session_state.edited_df is not None:
            # Create a container with custom styling to remove blank space
            st.markdown('<div class="classification-editor-container">', unsafe_allow_html=True)
            
            st.subheader("📝 Edit Classification Report")
            st.info("💡 **Auto-save is enabled**: All changes are automatically saved to the backend tables as you edit.")
            ...
            # Create the data editor with enhanced dimensions
            st.markdown('<div class="classification-dataframe">', unsafe_allow_html=True)
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "HIPAA_CLASS": st.column_config.TextColumn("HIPAA Class", width="medium"),
                    "BU_APPROVAL_STATUS": st.column_config.SelectboxColumn("BU Approval", width="medium"),
                    "BU_COMMENTS": st.column_config.TextColumn("BU Comments", width="large"),
                    "BU_ASSIGNEE": st.column_config.TextColumn("BU Assignee", width="medium"),
                    "INFOSEC_APPROVAL_STATUS": st.column_config.SelectboxColumn("InfoSec Approval", width="medium"),
                    "INFOSEC_APPROVER": st.column_config.TextColumn("InfoSec Approver", width="medium"),
                    "INFOSEC_COMMENTS": st.column_config.TextColumn("InfoSec Comments", width="large"),
                }
            )
            
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Check if data has been edited and auto-save (without refreshing)
            ...
            # BU selection and submit in columns
            ...
            with col2:
                if bu_name:
                    if st.button("Submit Classifications", use_container_width=True, type="primary"):
