...
st.markdown(
    ...
    /* Custom styling for larger dataframe */
    .dataframe-container {
        width: 100%;
        height: 80vh;
    }
...
if app_mode == "Classifications":
    ...
    # Main UI
    #st.sidebar.title("Snowflake Masking Validation")
    ...
    if app_mode_classification == "Home":
        ...
        st.markdown('<p>You can edit the classification report multiple times. The data saves automatically as you edit.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<p>Note: The classification report saves automatically as you edit, but it must be officially submitted only once to complete the process. </p>', unsafe_allow_html=True)
        ...
        for key in ["report_fetched", "edited_df", "submitted", "confirm_submission", "database", "schema", "auto_save_counter"]:
            if key not in st.session_state:
                if key == "edited_df":
                    st.session_state[key] = None
                elif key in ["database", "schema"]:
                    st.session_state[key] = ""
                elif key == "auto_save_counter":
        ...
        def save_classification_report(df, database, schema):
            ...
            try:
                ...
                st.error(f"Error saving classification report: {e}")
                return False
        ...
        database = st.selectbox("Select Database", fetch_databases(), 
                                ...
                                index=fetch_databases().index(st.session_state.database) if st.session_state.database in fetch_databases() else 0)
        
        if database != st.session_state.database:
            st.session_state.database = database
            st.session_state.report_fetched = False
            st.session_state.edited_df = None
            
        if database:
            schema_list = fetch_schemas(database)
            schema = st.selectbox("Select Schema", schema_list,
                                 key="schema_selector", 
                                 index=schema_list.index(st.session_state.schema) if st.session_state.schema in schema_list else 0)
            ...
            if schema and st.button("Get Classification Report"):
                ...
                if data:
                    ...
                    # NEW LOGIC: Use SQL to reliably get current user
                    try:
        ...
        if st.session_state.report_fetched and st.session_state.edited_df is not None:
            ...
            # Custom CSS for full-width, full-height dataframe
            st.markdown("""
                <style>
                .dataframe-container {
                    width: 100%;
                ...
                .stDataEditor [data-testid="stDataEditor"] > div {
                    width: 100% !important;
                    height: 80vh !important;
                }
            ...
            # Large dataframe editor with automatic saving
            edited_df = st.data_editor(
                st.session_state.edited_df, 
                num_rows="dynamic", 
                use_container_width=True,
                height=600,  # Set explicit height
                key=f"dataframe_editor_{st.session_state.auto_save_counter}"
            )

            # Check if dataframe has been modified and auto-save
            if not edited_df.equals(st.session_state.edited_df):
                # Auto-save the changes
                success = save_classification_report(edited_df, st.session_state.database, st.session_state.schema)
                if success:
                    # Update the session state with the new dataframe
                    st.session_state.edited_df = edited_df
                    st.session_state.auto_save_counter += 1
                    
                    # Show auto-save notification without refreshing
                    with status_container:
                        st.success("✅ Changes saved automatically!", icon="💾")
                    ...
                    with status_container:
                        st.error("❌ Auto-save failed. Please try again.", icon="⚠️")
